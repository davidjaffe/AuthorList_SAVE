...
def parse_tex_line_author(line):
    ...
    if map_tex_author.has_key(raw_name):
        ea_ = map_tex_author[raw_name]
        print "author name conflict: ", raw_name,'with affis',affis,'conflicts with raw name',ea_.raw_name,'affis',ea_.affis
        ...
        for line in f:
            ...
            for k,v in map_tex_affi.iteritems():
                if v.aff == l or v.aff.find(l) != -1 or l.find(v.aff) != -1:
                    ...
                    found = True
                elif l.find("Chile") != -1 and v.aff.find("Chile") != -1:
                    # CUC
                    map_tex_affi[k].xml_short = s
                    found = True
